...
import {
  ...
  getUniqueCodes,
  WOOD_VENEER_CONTACT,
  type WoodVeneerProduct,
} from "@/lib/wood-veneer-data";
import { categories } from "@/lib/products";
import styles from "./wood-veneer.module.css";

// ===== Types =====
interface QuoteItem {
  product: WoodVeneerProduct;
  quantity: number;
}
...
function buildGallery(product: WoodVeneerProduct): GalleryImage[] {
...
// ===== Format unit price (handles null) =====
function formatUnitPrice(product: WoodVeneerProduct): string {
  if (product.unitPrice === null) return "Contact for quote";
  return `$${product.unitPrice.toFixed(2)} / ${product.unit}`;
}

// ===== Main Component =====
export default function WoodVeneerClient() {
  // Filter state
  ...
  // Drawer state
  const [selectedProduct, setSelectedProduct] = useState<WoodVeneerProduct | null>(null);
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("wood-veneer-panel-quote-list");
      if (saved) {
  ...
  useEffect(() => {
    if (hydrated) {
      try {
        localStorage.setItem("wood-veneer-panel-quote-list", JSON.stringify(quoteList));
      } catch {
        // ignore
      }
    }
  ...
  const filteredProducts = useMemo(() => {
    return woodVeneerProducts.filter((p) => {
  ...
  // ===== Quote list handlers =====
  const handleAddToQuote = useCallback((product: WoodVeneerProduct, quantity = 1) => {
    setQuoteList((prev) => {
  ...
  const handleViewDetails = (product: WoodVeneerProduct) => {
  ...
  const handleDownloadInfo = (product: WoodVeneerProduct) => {
    const priceLine =
      product.unitPrice === null
        ? "Unit Price: Contact for quote"
        : `Unit Price: $${product.unitPrice.toFixed(2)} / ${product.unit}`;
...
Contact:
OneStopBuildly
Email: ${WOOD_VENEER_CONTACT.email}
WhatsApp: ${WOOD_VENEER_CONTACT.whatsapp}
...
`.trim();
  ...
  const relatedCategories = categories.filter((c) =>
    ["indoor-wpc-wall-panels", "3d-wall-panels", "acoustic-wall-panels"].includes(c.slug),
  );
        ...
        <div className="container">
          <div className={styles.introGrid}>
            <div className={styles.introText}>
              <span className="eyebrow">Wood Veneer Panels</span>
              <h1 className={styles.introTitle}>Bamboo Fiber &amp; Melamine MDF Wood Veneer Wall Panels</h1>
              <p className={styles.introDesc}>
                Real wood veneer laminated on WPC, MDF and bamboo fiber cores. Waterproof,
                melamine-faced, fancy wood grain finishes for furniture, interior decoration and
                feature walls. Available in 3mm–25mm thicknesses.
              </p>
              <div className={styles.introTags}>
                <span className={styles.tag}>Bamboo Fiber</span>
                <span className={styles.tag}>Melamine MDF</span>
                <span className={styles.tag}>Waterproof</span>
                <span className={styles.tag}>Fancy Wood Veneer</span>
            ...
            <div className={styles.introCollage}>
              <div className={styles.introCollageMain}>
                <Image
                  src="/images/products/wood-veneer/WV-001.jpg"
                  alt="Bamboo fiber wood veneer wall panel with natural grain finish"
                  fill
              ...
              <div className={styles.introCollageSide}>
                <div className={styles.introCollageSmall}>
                  <Image
                    src="/images/products/wood-veneer/WV-005.jpg"
                    alt="Melamine MDF wood veneer panel with fancy wood grain"
                    fill
                ...
                <div className={styles.introCollageSmall}>
                  <Image
                    src="/images/products/wood-veneer/WV-015.jpg"
                    alt="Waterproof wood veneer wall panel for interior decoration"
                    fill
                    sizes="(max-width: 960px) 100vw, 15vw"
                    className={styles.collageImg}
                  />
          ...
          <div className={styles.ctaInner}>
            <div className={styles.ctaText}>
              ...
              <p className={styles.ctaContact}>
                Email: <a href={`mailto:${WOOD_VENEER_CONTACT.email}`}>{WOOD_VENEER_CONTACT.email}</a>
                <br />
                WhatsApp: {WOOD_VENEER_CONTACT.whatsapp}
              </p>
            </div>
                  ...
                  <div className={styles.formGrid}>
                    ...
                    <div className={styles.formGroup}>
                      ...
                      <input
                        ...
                        placeholder="e.g. WV-001, Wood Veneer Panel"
                      />
                    </div>
