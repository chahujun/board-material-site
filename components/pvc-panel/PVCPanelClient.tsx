...
import {
  ...
  pvcPanelFAQs,
  PVC_PANEL_CONTACT,
  type PVCPanelProduct,
} from "@/lib/pvc-panel-data";
import { categories } from "@/lib/products";
import styles from "./pvc-panel.module.css";

// ===== Types =====
interface QuoteItem {
  product: PVCPanelProduct;
  quantity: number;
}
...
function buildGallery(product: PVCPanelProduct): GalleryImage[] {
...
// ===== Main Component =====
export default function PVCPanelClient() {
  // Filter state
  ...
  // Drawer state
  const [selectedProduct, setSelectedProduct] = useState<PVCPanelProduct | null>(null);
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("pvc-ceiling-quote-list");
      if (saved) {
  ...
  useEffect(() => {
    if (hydrated) {
      try {
        localStorage.setItem("pvc-ceiling-quote-list", JSON.stringify(quoteList));
      } catch {
        // ignore
      }
    }
  ...
  const uniqueSizes = useMemo(() => [...new Set(pvcPanelProducts.map((p) => p.size))].sort(), []);
  const uniqueColors = useMemo(() => [...new Set(pvcPanelProducts.map((p) => p.color))].sort(), []);

  // ===== Filtered products =====
  const filteredProducts = useMemo(() => {
    return pvcPanelProducts.filter((p) => {
  ...
  // ===== Quote list handlers =====
  const handleAddToQuote = useCallback((product: PVCPanelProduct, quantity = 1) => {
    setQuoteList((prev) => {
  ...
  const handleViewDetails = (product: PVCPanelProduct) => {
  ...
  const handleDownloadInfo = (product: PVCPanelProduct) => {
    const info = `
Product Information
===================
...
Contact:
${PVC_PANEL_CONTACT.company}
Email: ${PVC_PANEL_CONTACT.email}
Phone/WhatsApp: ${PVC_PANEL_CONTACT.phone}
        ...
        <div className="container">
          <div className={styles.introGrid}>
            <div className={styles.introText}>
              <span className="eyebrow">Waterproof PVC ceiling and wall panels</span>
              <h1 className={styles.introTitle}>PVC Ceiling &amp; Wall Panels</h1>
              <p className={styles.introDesc}>
                Waterproof, fireproof and moisture-proof PVC ceiling and wall panels for
                bathrooms, kitchens, hotels and commercial spaces.
              </p>
              <div className={styles.introTags}>
                <span className={styles.tag}>Factory Direct</span>
                <span className={styles.tag}>Multiple Styles</span>
                <span className={styles.tag}>Project Supply</span>
              </div>
            </div>
            <div className={styles.introCollage}>
              <div className={styles.introCollageMain}>
                <Image
                  src="/images/products/pvc-panel/SC-2505.jpg"
                  alt="PVC ceiling panel with wood grain finish"
                  fill
              ...
              <div className={styles.introCollageSide}>
                <div className={styles.introCollageSmall}>
                  <Image
                    src="/images/products/pvc-panel/GP-6001.jpg"
                    alt="Glossy printing PVC ceiling tile with decorative pattern"
                    fill
                ...
                <div className={styles.introCollageSmall}>
                  <Image
                    src="/images/products/pvc-panel/LW-004.jpg"
                    alt="Laminated wood grain PVC wall panel"
                    fill
                    sizes="(max-width: 960px) 100vw, 15vw"
                    className={styles.collageImg}
                  />
            ...
            <div className={styles.drawerBody}>
              ...
              <div className={styles.detailSpecs}>
                <div className={styles.specItem}>
                  ...
                  <span className={styles.specValue}>{selectedProduct.size}mm</span>
                </div>
                <div className={styles.specItem}>
                    ...
                    <div key={item.product.code} className={styles.quoteItem}>
                      ...
                      <div className={styles.quoteItemInfo}>
                        ...
                        <span className={styles.quoteItemCode}>
                          Code: {item.product.code} · {item.product.size}mm · {item.product.color}
                        </span>
          ...
          <div className={styles.ctaInner}>
            <div className={styles.ctaText}>
              ...
              <p className={styles.ctaContact}>
                Email: <a href={`mailto:${PVC_PANEL_CONTACT.email}`}>{PVC_PANEL_CONTACT.email}</a>
                <br />
                WhatsApp: {PVC_PANEL_CONTACT.whatsapp}
              </p>
            </div>
                  ...
                  <div className={styles.formGrid}>
                    ...
                    <div className={styles.formGroup}>
                      ...
                      <input
                        ...
                        placeholder="e.g. Grey, White, Custom"
                      />
                    </div>
