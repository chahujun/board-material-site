...
import {
  ...
  PU_STONE_CONTACT,
  type PUStoneProduct,
} from "@/lib/pu-stone-data";
import { categories } from "@/lib/products";
import styles from "./pu-stone.module.css";

// ===== Types =====
interface QuoteItem {
  product: PUStoneProduct;
  quantity: number;
}
...
function buildGallery(product: PUStoneProduct): GalleryImage[] {
...
// ===== Main Component =====
export default function PUStoneClient() {
  // Filter state
  ...
  // Drawer state
  const [selectedProduct, setSelectedProduct] = useState<PUStoneProduct | null>(null);
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("pu-stone-quote-list");
      if (saved) {
  ...
  useEffect(() => {
    if (hydrated) {
      try {
        localStorage.setItem("pu-stone-quote-list", JSON.stringify(quoteList));
      } catch {
        // ignore
      }
    }
  ...
  const uniqueSizes = useMemo(() => [...new Set(puStoneProducts.map((p) => p.size))].sort(), []);
  const uniqueColors = useMemo(() => [...new Set(puStoneProducts.map((p) => p.color))].sort(), []);

  // ===== Filtered products =====
  const filteredProducts = useMemo(() => {
    return puStoneProducts.filter((p) => {
  ...
  // ===== Quote list handlers =====
  const handleAddToQuote = useCallback((product: PUStoneProduct, quantity = 1) => {
    setQuoteList((prev) => {
  ...
  const handleViewDetails = (product: PUStoneProduct) => {
  ...
  const handleDownloadInfo = (product: PUStoneProduct) => {
    const info = `
Product Information
===================
...
Contact:
${PU_STONE_CONTACT.company}
Email: ${PU_STONE_CONTACT.email}
Phone/WhatsApp: ${PU_STONE_CONTACT.phone}
...
`.trim();
  ...
  // ===== Related categories =====
  const relatedCategories = categories.filter((c) => c.slug !== "pu-stone-panels").slice(0, 3);
        ...
        <div className="container">
          <div className={styles.introGrid}>
            <div className={styles.introText}>
              <span className="eyebrow">Polyurethane faux stone panels</span>
              <h1 className={styles.introTitle}>PU Stone Panels</h1>
              <p className={styles.introDesc}>
                Lightweight polyurethane stone panels designed to replicate natural stone
                textures for interior walls, exterior facades and decorative spaces.
              </p>
            ...
            <div className={styles.introImage}>
              <SmartImage
                src="/images/products/pu-stone/category-hero.jpg"
                alt="PU Stone Panels — polyurethane faux stone for interior and exterior"
                fill
          ...
          <div className={styles.ctaInner}>
            <div className={styles.ctaText}>
              ...
              <p className={styles.ctaContact}>
                Email: <a href={`mailto:${PU_STONE_CONTACT.email}`}>{PU_STONE_CONTACT.email}</a>
                <br />
                WhatsApp: {PU_STONE_CONTACT.whatsapp}
              </p>
            </div>
