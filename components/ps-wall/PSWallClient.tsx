...
import {
  ...
  PS_WALL_CONTACT,
  type PSWallProduct,
} from "@/lib/ps-wall-data";
import { categories } from "@/lib/products";
import styles from "./ps-wall.module.css";

// ===== Types =====
interface QuoteItem {
  product: PSWallProduct;
  quantity: number;
}
...
function buildGallery(product: PSWallProduct): GalleryImage[] {
...
// ===== Main Component =====
export default function PSWallClient() {
  // Filter state
  ...
  // Drawer state
  const [selectedProduct, setSelectedProduct] = useState<PSWallProduct | null>(null);
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("ps-wall-panel-quote-list");
      if (saved) {
  ...
  useEffect(() => {
    if (hydrated) {
      try {
        localStorage.setItem("ps-wall-panel-quote-list", JSON.stringify(quoteList));
      } catch {
        // ignore
      }
    }
  ...
  const filteredProducts = useMemo(() => {
    return psWallProducts.filter((p) => {
  ...
  // ===== Quote list handlers =====
  const handleAddToQuote = useCallback((product: PSWallProduct, quantity = 1) => {
    setQuoteList((prev) => {
  ...
  const handleViewDetails = (product: PSWallProduct) => {
  ...
  const handleDownloadInfo = (product: PSWallProduct) => {
    const info = `
Product Information
===================
...
Contact:
OneStopBuildly
Email: ${PS_WALL_CONTACT.email}
WhatsApp: ${PS_WALL_CONTACT.whatsapp}
...
`.trim();
  ...
  const relatedCategories = categories.filter((c) =>
    ["indoor-wpc-wall-panels", "3d-wall-panels", "wood-veneer-panels"].includes(c.slug),
  );
        ...
        <div className="container">
          <div className={styles.introGrid}>
            <div className={styles.introText}>
              ...
              <h1 className={styles.introTitle}>PS Wall Panels</h1>
            ...
            <div className={styles.introCollage}>
              <div className={styles.introCollageMain}>
                <Image
                  src="/images/products/ps-wall/PS-001.jpg"
              ...
              <div className={styles.introCollageSide}>
                <div className={styles.introCollageSmall}>
                  <Image
                    src="/images/products/ps-wall/PS-007.jpg"
                ...
                <div className={styles.introCollageSmall}>
                  <Image
                    src="/images/products/ps-wall/PS-016.jpg"
          ...
          <div className={styles.ctaInner}>
            <div className={styles.ctaText}>
              ...
              <p className={styles.ctaContact}>
                Email: <a href={`mailto:${PS_WALL_CONTACT.email}`}>{PS_WALL_CONTACT.email}</a>
                <br />
                WhatsApp: {PS_WALL_CONTACT.whatsapp}
              </p>
            </div>
