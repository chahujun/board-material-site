...
import {
  ...
  getUniqueColors,
  getUniqueCodes,
  THREE_D_WALL_CONTACT,
  type ThreeDWallProduct,
} from "@/lib/3d-wall-data";
import { categories } from "@/lib/products";
import styles from "./3d-wall.module.css";

// ===== Types =====
interface QuoteItem {
  product: ThreeDWallProduct;
  quantity: number;
}
...
function buildGallery(product: ThreeDWallProduct): GalleryImage[] {
...
// ===== Main Component =====
export default function ThreeDWallClient() {
  // Filter state
  ...
  // Drawer state
  const [selectedProduct, setSelectedProduct] = useState<ThreeDWallProduct | null>(null);
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("3d-wall-panel-quote-list");
      if (saved) {
  ...
  useEffect(() => {
    if (hydrated) {
      try {
        localStorage.setItem("3d-wall-panel-quote-list", JSON.stringify(quoteList));
      } catch {
        // ignore
      }
    }
  ...
  const filteredProducts = useMemo(() => {
    return threeDWallProducts.filter((p) => {
  ...
  // ===== Quote list handlers =====
  const handleAddToQuote = useCallback((product: ThreeDWallProduct, quantity = 1) => {
    setQuoteList((prev) => {
  ...
  const handleViewDetails = (product: ThreeDWallProduct) => {
  ...
  const handleDownloadInfo = (product: ThreeDWallProduct) => {
    const info = `
Product Information
===================
...
Contact:
OneStopBuildly
Email: ${THREE_D_WALL_CONTACT.email}
WhatsApp: ${THREE_D_WALL_CONTACT.whatsapp}
...
`.trim();
  ...
  const relatedCategories = categories.filter((c) =>
    ["pvc-ceiling-panels", "uv-marble-sheets", "acoustic-wall-panels", "3d-wall-panels"].includes(c.slug),
  );
        ...
        <div className="container">
          <div className={styles.introGrid}>
            <div className={styles.introText}>
              <span className="eyebrow">3D PVC decorative panels for modern interior accent walls</span>
              <h1 className={styles.introTitle}>3D Wall Panels</h1>
              <p className={styles.introDesc}>
                3D PVC decorative wall panels with diamond, geometric and art deco patterns.
                Waterproof, fireproof and soundproof PVC panels for luxurious interior wall
            ...
            <div className={styles.introCollage}>
              <div className={styles.introCollageMain}>
                <Image
                  src="/images/products/3d-wall/3D-001.jpg"
                  alt="3D geometric diamond wall panel with metallic finish"
                  fill
              ...
              <div className={styles.introCollageSide}>
                <div className={styles.introCollageSmall}>
                  <Image
                    src="/images/products/3d-wall/3D-002.jpg"
                    alt="Shining gold diamond 3D wall panel"
                    fill
                ...
                <div className={styles.introCollageSmall}>
                  <Image
                    src="/images/products/3d-wall/3D-010.jpg"
                    alt="Golden mirror 3D decorative wall panel"
                    fill
                    sizes="(max-width: 960px) 100vw, 15vw"
                    className={styles.collageImg}
                  />
          ...
          <div className={styles.ctaInner}>
            <div className={styles.ctaText}>
              ...
              <p className={styles.ctaContact}>
                Email: <a href={`mailto:${THREE_D_WALL_CONTACT.email}`}>{THREE_D_WALL_CONTACT.email}</a>
                <br />
                WhatsApp: {THREE_D_WALL_CONTACT.whatsapp}
              </p>
            </div>
                  ...
                  <div className={styles.formGrid}>
                    ...
                    <div className={styles.formGroup}>
                      ...
                      <input
                        ...
                        placeholder="e.g. 3D-001, 3D Wall Panel"
                      />
                    </div>
