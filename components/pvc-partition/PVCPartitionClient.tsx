...
import {
  ...
  getUniqueColors,
  getUniqueCodes,
  PVC_PARTITION_CONTACT,
  type PVCPartitionProduct,
} from "@/lib/pvc-partition-data";
import { categories } from "@/lib/products";
import styles from "./pvc-partition.module.css";

// ===== Types =====
interface QuoteItem {
  product: PVCPartitionProduct;
  quantity: number;
}
...
function buildGallery(product: PVCPartitionProduct): GalleryImage[] {
...
// ===== Main Component =====
export default function PVCPartitionClient() {
  // Filter state
  ...
  // Drawer state
  const [selectedProduct, setSelectedProduct] = useState<PVCPartitionProduct | null>(null);
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("pvc-partition-quote-list");
      if (saved) {
  ...
  useEffect(() => {
    if (hydrated) {
      try {
        localStorage.setItem("pvc-partition-quote-list", JSON.stringify(quoteList));
      } catch {
        // ignore
      }
    }
  ...
  const filteredProducts = useMemo(() => {
    return pvcPartitionProducts.filter((p) => {
  ...
  // ===== Quote list handlers =====
  const handleAddToQuote = useCallback((product: PVCPartitionProduct, quantity = 1) => {
    setQuoteList((prev) => {
  ...
  const handleViewDetails = (product: PVCPartitionProduct) => {
  ...
  const handleDownloadInfo = (product: PVCPartitionProduct) => {
    const info = `
Product Information
===================
...
Contact:
OneStopBuildly
Email: ${PVC_PARTITION_CONTACT.email}
WhatsApp: ${PVC_PARTITION_CONTACT.whatsapp}
...
`.trim();
  ...
  const relatedCategories = categories.filter((c) =>
    ["pvc-marble-sheet", "indoor-wpc-wall-panels", "3d-wall-panels"].includes(c.slug),
  );
          ...
          <div className={styles.introGrid}>
            <div className={styles.introText}>
              ...
              <div className={styles.introTags}>
                ...
                <span className={styles.tag}>Fireproof</span>
                <span className={styles.tag}>Moisture-Proof</span>
                <span className={styles.tag}>Sound-Absorbing</span>
                <span className={styles.tag}>Easy Install</span>
            ...
            <div className={styles.introCollage}>
              <div className={styles.introCollageMain}>
                <Image
                  src="/images/products/pvc-partition-panel/PP-001.jpg"
                  alt="PVC partition panel for office divider"
                  fill
              ...
              <div className={styles.introCollageSide}>
                <div className={styles.introCollageSmall}>
                  <Image
                    src="/images/products/pvc-partition-panel/PP-006.jpg"
                    alt="Double laminate partition panel surface"
                    fill
                ...
                <div className={styles.introCollageSmall}>
                  <Image
                    src="/images/products/pvc-partition-panel/PP-012.jpg"
                    alt="Decorative PVC partition wall panel"
                    fill
                    sizes="(max-width: 960px) 100vw, 15vw"
                    className={styles.collageImg}
                  />
          ...
          <div className={styles.ctaInner}>
            <div className={styles.ctaText}>
              ...
              <p className={styles.ctaContact}>
                Email: <a href={`mailto:${PVC_PARTITION_CONTACT.email}`}>{PVC_PARTITION_CONTACT.email}</a>
                <br />
                WhatsApp: {PVC_PARTITION_CONTACT.whatsapp}
              </p>
            </div>
