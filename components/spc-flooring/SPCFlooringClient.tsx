...
import {
  ...
  getUniqueColors,
  getUniqueCodes,
  SPC_FLOORING_CONTACT,
  type SPCFlooringProduct,
} from "@/lib/spc-flooring-data";
import { categories } from "@/lib/products";
import styles from "./spc-flooring.module.css";

// ===== Types =====
interface QuoteItem {
  product: SPCFlooringProduct;
  quantity: number;
}
...
function buildGallery(product: SPCFlooringProduct): GalleryImage[] {
...
// ===== Main Component =====
export default function SPCFlooringClient() {
  // Filter state
  ...
  // Drawer state
  const [selectedProduct, setSelectedProduct] = useState<SPCFlooringProduct | null>(null);
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("spc-flooring-quote-list");
      if (saved) {
  ...
  useEffect(() => {
    if (hydrated) {
      try {
        localStorage.setItem("spc-flooring-quote-list", JSON.stringify(quoteList));
      } catch {
        // ignore
      }
    }
  ...
  const filteredProducts = useMemo(() => {
    return spcFlooringProducts.filter((p) => {
  ...
  // ===== Quote list handlers =====
  const handleAddToQuote = useCallback((product: SPCFlooringProduct, quantity = 1) => {
    setQuoteList((prev) => {
  ...
  const handleViewDetails = (product: SPCFlooringProduct) => {
  ...
  const handleDownloadInfo = (product: SPCFlooringProduct) => {
    const info = `
Product Information
===================
...
Contact:
OneStopBuildly
Email: ${SPC_FLOORING_CONTACT.email}
WhatsApp: ${SPC_FLOORING_CONTACT.whatsapp}
...
`.trim();
  ...
  const relatedCategories = categories.filter((c) =>
    ["indoor-wpc-wall-panels", "wpc-door", "wood-veneer-panels"].includes(c.slug),
  );
        ...
        <div className="container">
          <div className={styles.introGrid}>
            <div className={styles.introText}>
              ...
              <h1 className={styles.introTitle}>SPC Flooring</h1>
            ...
            <div className={styles.introCollage}>
              <div className={styles.introCollageMain}>
                <Image
                  src="/images/products/spc-flooring/SF-001.jpg"
                  alt="PS geometric diamond wall panel with metallic finish"
                  fill
              ...
              <div className={styles.introCollageSide}>
                <div className={styles.introCollageSmall}>
                  <Image
                    src="/images/products/spc-flooring/SF-006.jpg"
                    alt="Shining gold diamond PS wall panel"
                    fill
                ...
                <div className={styles.introCollageSmall}>
                  <Image
                    src="/images/products/spc-flooring/SF-008.jpg"
                    alt="Golden mirror PS decorative wall panel"
                    fill
                    sizes="(max-width: 960px) 100vw, 15vw"
                    className={styles.collageImg}
                  />
          ...
          <div className={styles.ctaInner}>
            <div className={styles.ctaText}>
              ...
              <p className={styles.ctaContact}>
                Email: <a href={`mailto:${SPC_FLOORING_CONTACT.email}`}>{SPC_FLOORING_CONTACT.email}</a>
                <br />
                WhatsApp: {SPC_FLOORING_CONTACT.whatsapp}
              </p>
            </div>
